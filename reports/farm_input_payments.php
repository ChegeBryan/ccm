...
class PDF extends FPDF
{
  ...
  function Header()
  {
    ...
    $this->Cell(120);
    // Title
    $this->Cell(30, 10, 'CCM fertilizer payments', 0, 0, 'R');
    // Line break
    $this->Ln(13);
  }
  ...
  function ScheduledAppointmentsReport($header, $conn)
  {
    ...
    $sql = "SELECT quantity, pick_date, confirmed_on, fullname, total_cost, ccm_farm_inputs.farm_input  \n"
    ...
    if ($result->num_rows > 0) {
      ...
      while ($row = $result->fetch_array()) {
        ...
        $this->Cell($w[5], 9, date('M-d-Y', strtotime($row['pick_date'])), 'LR', 0, 'R', $fill);
        $this->Ln();
        $fill = !$fill;
        $n++;
      }
...
$pdf = new PDF();
// Column headings
$header = array('#', 'Farmer',  'Picked', 'Quantity (Kgs)', 'Cost (Kshs)', 'Picked on');

$pdf->AliasNbPages();
$pdf->SetFont('Arial', '', 11);
$pdf->AddPage();
$pdf->ScheduledAppointmentsReport($header, $conn);
$pdf->Output('D', 'CCM_FARMERS_FERTILIZER_PAYMENTS_REPORT_' . date("m/d/Y H:i:s") . '.pdf');
